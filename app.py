...
@app.route('/works/sorting_algorithms')
def sorting_algorithm():
    return render_template('sorting_algorithms.html', active_page='sorting_algorithm')


@app.route("/works/sorting_algorithms/bubble_sort", methods=["GET", "POST"])
def bubble_sort_page():
    ...
    return render_template("bubble_sort.html", steps=steps)

merge_sort_array = []

@app.route('/works/sorting_algorithms/merge-sort-visualizer', methods=['GET', 'POST'])
def merge_sort_visualizer():
    global merge_sort_array
    if request.method == "POST":
        value = request.form.get("value")

        if "enter" in request.form and value:
            try:
                merge_sort_array.append(int(value))
            except ValueError:
                pass
        elif "sort" in request.form and merge_sort_array:
